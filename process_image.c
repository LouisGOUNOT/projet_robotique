...
static THD_FUNCTION(CaptureImage, arg) {
	...
	//Initialise la hauteur pour ligne noire au d�marrage
...
//					chSysLock();
					...
					// si trouve bleu quand c'est demand� alors arrete de tourner et �teint la led et moteur
					// si pas demand� alors reset le compteur de rouge pour �viter les faux positifs
					if((meanRatio < 3)&&(meanRatio  > 0))
					{
						if (target_color == 0){
							...
							i_blue++;
							if (((get_line_position() - (IMAGE_BUFFER_SIZE/2))<70)){ //5 est une valeur experimentale a peaufiner pour �viter les erreurs
								set_rgb_led(LED2,0,0,0);
								set_rgb_led(LED4,0,0,0);
								pxtocm = PXTOCM_COLOR;
								set_rgb_led(LED8,0,255,0);
								chprintf((BaseSequentialStream *)&SD3, "JE SUIS DANS DETECTION BLEU");
								// COPIE COLL� DES ACTIONS QUAND TROUVE
...
//								}
					...
					// si trouve rouge quand c'est demand� alors arrete de tourner et �teint la led et moteurs
					else if((meanRatio  > 2))
					{
							i_red++;
	...
	switch (color_id){
			...
			case 0:
				if(target_color == 1){
			    	...
			    	//Capture une image pour r�actualiser le buffer et �viter de detecter la ligne noire d�s le debut
			        chBSemWait(&image_ready_sem);
			...
			case 1:
				target_color = 1;
				camera_height = 460;
				//r�gule contraste avec cste 0< <255
